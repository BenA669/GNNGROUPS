...
def train_one_epoch_better(model, dataloader, optimizer, device, infonce_loss_fn, timesteps):
    model.train()
    epoch_loss=0.0
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader)):
        ...
        # Backward pass through the whole sequence
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
...
@torch.no_grad()
def validate_one_epoch(model, dataloader, device, infonce_loss_fn, timesteps):
    model.eval()
    epoch_loss = 0.0
...
def main():
    ...
    model = TrainOT(config=config).to(device)
    ...
    for epoch in range(1, epochs+1):
        train_loss = train_one_epoch_better(model, train_loader, optimizer, device, infonce_loss_fn, timesteps)
        val_loss = validate_one_epoch(model, val_loader, device, infonce_loss_fn, timesteps)
